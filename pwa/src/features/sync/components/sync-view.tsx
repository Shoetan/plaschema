...
import { useDeviceSyncState, useEnrollmentSync, useLocalEnrollments } from '@/features/enrollment/hooks'
...
export function SyncView() {
  ...
  const syncState = useDeviceSyncState(user.id)
  ...
  const needsFinalReport = Boolean(syncState?.needsReport)
  ...
  return <div className="space-y-5 px-4 py-6">
    ...
    <section className="card space-y-4 p-4">
      ...
      <div className="rounded-lg bg-amber-50 p-3"><p className="text-xs text-amber-700">Needs sync</p><p className="text-2xl font-bold text-amber-800">{pending.length}</p>{needsFinalReport && <p className="mt-1 text-xs font-semibold text-amber-700">Finishing a completed sync report</p>}</div>
      <button className="primary-button flex w-full items-center justify-center gap-2" disabled={!isOnline || activelySyncing || (retryable === 0 && !needsFinalReport)} onClick={() => sync.mutate()}><RefreshCw className={activelySyncing ? 'animate-spin' : ''} size={17} />{activelySyncing ? 'Synchronizing…' : retryable || needsFinalReport ? 'Sync now' : pending.length ? 'Review failed records' : 'Everything is synced'}</button>
