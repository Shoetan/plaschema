import { Ellipsis, Eye, Power, PowerOff } from 'lucide-react'
...
export function EnrollmentRowActions({ enrollment, onView, onAction }: EnrollmentRowActionsProps) {
  const targets = availableEnrollmentStatusTargets(enrollment.status)

  return (
    <div className="flex items-center gap-1">
      <Button aria-label={`View ${enrollment.beneficiaryName}`} onClick={onView} size="icon-sm" variant="ghost"><Eye aria-hidden="true" className="size-4" /></Button>
      {targets.length > 0 && <DropdownMenu.Root>
