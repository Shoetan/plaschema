  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
      ...
      <div className="flex flex-wrap items-end gap-3">
        <label className="grid gap-1 text-xs font-medium text-muted-foreground">Month<select aria-label="Capitation month" className="h-10 rounded-lg border border-border bg-card px-3 text-sm font-normal text-foreground" onChange={(event) => { setMonth(Number(event.target.value)); resetPage() }} value={month}>{CAPITATION_MONTHS.map((label, index) => <option key={label} value={index + 1}>{label}</option>)}</select></label>
        <label className="grid gap-1 text-xs font-medium text-muted-foreground">Year<select aria-label="Capitation year" className="h-10 rounded-lg border border-border bg-card px-3 text-sm font-normal text-foreground" onChange={(event) => { setYear(Number(event.target.value)); resetPage() }} value={year}>{YEARS.map((value) => <option key={value}>{value}</option>)}</select></label>
