...
const navItems = [
  ...
  { to: '/enroll-household', label: 'Enroll', icon: Plus, primary: true },
...
export function AppLayout() {
  ...
  const isEnrollmentRoute = useLocation().pathname.startsWith('/enroll-household') || useLocation().pathname.includes('/add-member')
  return (
    <div className="app-frame flex flex-col">
      ...
      <UpdatePrompt />
      <main className={`min-h-0 flex-1 ${isEnrollmentRoute ? 'overflow-hidden' : 'overflow-y-auto'}`}>{isEnrollmentRoute ? <div className="h-full min-h-0"><Outlet /></div> : <Outlet />}</main>
      {!isEnrollmentRoute ? (
