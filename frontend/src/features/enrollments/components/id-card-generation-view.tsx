...
import { BENEFICIARY_CATEGORIES, formatEnrollmentDate, statusLabel } from '../utils'
...
export function IdCardGenerationView() {
  ...
  return <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
    ...
    <div className={`overflow-hidden rounded-xl bg-card ${cardShadow}`}>
      ...
      {!query.isError && <div className="flex items-center justify-between border-t border-border px-4 py-3"><p className="text-sm text-muted-foreground">Page {pageIndex + 1} · Showing {rows.length} of {totalCount.toLocaleString()} · {selected.size} of 9 selected</p><div className="flex gap-2"><Button disabled={pageIndex === 0 || query.isFetching} onClick={() => setPageIndex((current) => Math.max(0, current - 1))} variant="outline"><ChevronLeft aria-hidden="true" /> Previous</Button><Button disabled={!meta?.hasMore || !meta.nextCursor || query.isFetching} onClick={nextPage} variant="outline">Next <ChevronRight aria-hidden="true" /></Button></div></div>}
    </div>
  </div>
}
