...
import { BENEFICIARY_CATEGORIES, hasDraftProgress } from '../utils'
...
export function EnrollmentView() {
  ...
  function validateCurrentStep() {
    ...
    const needed: Array<keyof EnrollmentFormValues> = draft.step === 0
      ...
      : draft.step === 1 ? ['stateOfResidence', 'lgaOfResidence', 'residentialAddress', 'wardId']
      : draft.step === 2 ? ['phone']
    ...
    if (form.phone.length > 30 || form.emergencyPhone.length > 30) nextErrors.phone = 'Phone numbers must be 30 characters or fewer.'
    if (form.nin.length > 20) nextErrors.nin = 'NIN must be 20 characters or fewer.'
  ...
  return <div className="flex min-h-full flex-col">
    ...
    <footer className="sticky bottom-[69px] z-20 flex gap-3 border-t border-neutral-200 bg-white px-4 py-3">{step > 0 && <button className="secondary-button flex-1" onClick={() => changeStep(step - 1)}>Back</button>}<button className="primary-button flex-[2]" disabled={isQueueing} onClick={() => void continueFlow()}>{step === 5 ? isQueueing ? 'Saving…' : 'Save enrollment' : 'Save & continue'}</button></footer>
  </div>
}
