...
export function BeneficiariesView() {
  ...
  const filtered = useMemo(() => enrollments.filter((record) => record.syncStatus !== 'synced').filter((record) => {
    ...
    const group: Filter = record.syncStatus === 'failed' ? 'failed' : 'pending'
  ...
  return <div className="px-4 py-6">
    ...
    <p className="mt-1 text-xs text-neutral-500">Shows pending and failed enrollments saved on this device.</p>
    ...
    <div className="-mx-4 mt-3 flex gap-2 overflow-x-auto px-4 pb-1" aria-label="Filter by sync state">{(['all', 'pending', 'failed'] as Filter[]).map((value) => <button key={value} onClick={() => setFilter(value)} className={`shrink-0 rounded-full px-3 py-2 text-xs font-bold capitalize ${filter === value ? 'bg-neutral-950 text-white' : 'bg-neutral-100 text-neutral-600'}`}>{value}</button>)}</div>
    {filtered.length === 0 ? <div className="flex flex-col items-center py-20 text-center"><span className="flex h-16 w-16 items-center justify-center rounded-full bg-neutral-100 text-neutral-400"><UserRoundPlus /></span><h2 className="mt-4 font-bold">No enrollments found</h2><p className="mt-1 text-sm text-neutral-500">Try another name or filter, or start a new household enrollment.</p><Link to="/enroll-household" className="primary-button mt-5">Enroll new Household</Link></div>
