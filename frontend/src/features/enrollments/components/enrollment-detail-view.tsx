import { ArrowLeft, ExternalLink, Pencil, Power, PowerOff, RefreshCw } from 'lucide-react'
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2"><StatusBadge status={statusLabel(record.status)} /><span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold">{record.hasPrinted ? `Printed ${record.printCount} time${record.printCount === 1 ? '' : 's'}` : 'ID card not printed'}</span><Button onClick={() => setEditOpen(true)} variant="outline"><Pencil aria-hidden="true" className="size-4" /> Edit beneficiary</Button>{statusTargets.map((target) => <Button key={target} onClick={() => setStatusAction({ bulk: false, enrollmentIds: [record.id], subject: `${overview.beneficiaryName} (${record.enrollmentId})`, target })} variant={target === 'disabled' ? 'destructive' : 'default'}>{target === 'active' ? <Power aria-hidden="true" /> : <PowerOff aria-hidden="true" />}{target === 'active' ? 'Activate' : 'Deactivate'}</Button>)}</div>
      </div>
      ...
      {editOpen && <EditEnrollmentDialog onOpenChange={setEditOpen} open record={record} />}
