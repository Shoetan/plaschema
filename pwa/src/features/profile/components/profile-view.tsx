...
export function ProfileView() {
  ...
  return <div className="space-y-5 px-4 py-6">
    ...
    <section className="card divide-y divide-neutral-100 px-4">{[['Phone', user.phone ?? 'Not provided'], ['Assigned wards', wardAccess], ['Last sync', formatDateTime(user.lastSyncedAt)], ['App version', 'PWA 1.0']].map(([label, value]) => <div className="flex justify-between gap-4 py-3 text-sm" key={label}><span className="shrink-0 text-neutral-500">{label}</span><span className="min-w-0 break-words text-right font-semibold">{value}</span></div>)}</section>
    ...
    <div className="space-y-3">
      <button className="primary-button flex w-full items-center justify-center gap-2" onClick={() => void syncAll()}><RefreshCw aria-hidden="true" size={17} />Run mock sync</button>
