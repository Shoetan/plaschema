...
function Field({ label, required, error, children }: { label: string; required?: boolean; error?: string; children: ReactNode }) {
  return <label className="flex min-w-0 max-w-full flex-col gap-1.5 text-sm font-bold">{label}{required && <span className="sr-only"> required</span>}{children}{error && <span className="text-xs font-semibold text-red-600">{error}</span>}</label>
}
...
export function EnrollmentView() {
  ...
  return <div className="flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div ref={fieldsScrollRef} className="min-h-0 w-full max-w-full flex-1 space-y-4 overflow-x-hidden overflow-y-auto overscroll-contain px-4 py-5">
