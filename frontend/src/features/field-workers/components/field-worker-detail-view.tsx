...
import { useFieldWorkerDetail, useUpdateFieldWorker } from '../hooks'
...
export function FieldWorkerDetailView({ fieldWorkerId }: FieldWorkerDetailViewProps) {
  ...
  const beneficiaryQuery = useEnrollments({ cursor: beneficiaryCursors[beneficiaryPage], enrolledByUserId: fieldWorkerId, limit: 25 }, tab === 'Beneficiaries Enrolled')
  const statusMutation = useUpdateFieldWorker()

  if (detailQuery.isPending) return <div className="flex flex-1 flex-col gap-6 p-6"><Skeleton className="h-6 w-64" /><div className="flex items-center gap-4"><Skeleton className="size-16 rounded-full" /><div className="space-y-2"><Skeleton className="h-7 w-56" /><Skeleton className="h-4 w-80" /></div></div><div className="grid grid-cols-2 gap-4 xl:grid-cols-4">{Array.from({ length: 4 }, (_, index) => <Skeleton className="h-24 rounded-xl" key={index} />)}</div><Skeleton className="h-72 rounded-xl" /></div>
  if (detailQuery.isError || !detailQuery.data) {
    const missing = getApiErrorStatus(detailQuery.error) === 404
    return <div className="flex flex-1 flex-col items-center justify-center gap-4 p-6 text-center" role="alert"><p className="text-lg font-semibold">{missing ? 'Enrollment officer not found' : 'Unable to load this enrollment officer'}</p><p className="max-w-md text-sm text-muted-foreground">{missing ? 'This account may have been removed or is not an enrollment officer account.' : 'Check your connection and try again.'}</p><div className="flex gap-2"><Button onClick={() => navigate('/admin/field-workers')} variant="outline">Back to Enrollment Officers</Button>{!missing && <Button onClick={() => void detailQuery.refetch()}><RefreshCw aria-hidden="true" /> Retry</Button>}</div></div>
  }
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
      <div className="flex items-center gap-2 text-sm text-muted-foreground"><button className="hover:text-foreground" onClick={() => navigate('/admin/field-workers')} type="button">Enrollment Officers</button><span>/</span><span className="font-medium text-foreground">{worker.name}</span></div>
      ...
      {tab === 'Wards' && <div className={`overflow-hidden rounded-xl bg-card ${cardShadow}`}>{detail.wards.length === 0 ? <div className="p-8 text-center"><p className="font-semibold">Access to all wards</p><p className="mt-1 text-sm text-muted-foreground">No ward restrictions are assigned to this enrollment officer.</p><Button className="mt-4" onClick={() => setEditOpen(true)} variant="outline">Change ward access</Button></div> : <div>{detail.wards.map((ward) => <div className="flex items-center justify-between border-b border-border px-5 py-4 last:border-0" key={ward.id}><div><p className="text-sm font-semibold">{ward.name}</p><p className="mt-0.5 text-xs text-muted-foreground">{ward.lga}{ward.state ? `, ${ward.state}` : ''}</p></div><StatusBadge status="Active" /></div>)}</div>}</div>}
      ...
      <AlertDialog onOpenChange={(nextOpen) => !statusMutation.isPending && setStatusOpen(nextOpen)} open={statusOpen}><AlertDialogContent><AlertDialogTitle>{worker.status === 'active' ? 'Deactivate enrollment officer?' : 'Activate enrollment officer?'}</AlertDialogTitle><AlertDialogDescription className="mt-2 text-sm text-muted-foreground">{worker.status === 'active' ? `${worker.name} will no longer be able to use an active enrollment officer account.` : `${worker.name} will regain access according to the assigned ward rules.`}</AlertDialogDescription><div className="mt-6 flex justify-end gap-3"><AlertDialogCancel asChild><Button disabled={statusMutation.isPending} variant="outline">Cancel</Button></AlertDialogCancel><AlertDialogAction asChild><Button disabled={statusMutation.isPending} onClick={changeStatus} variant={worker.status === 'active' ? 'destructive' : 'default'}>{statusMutation.isPending ? <><LoaderCircle aria-hidden="true" className="animate-spin" /> Saving…</> : worker.status === 'active' ? 'Deactivate' : 'Activate'}</Button></AlertDialogAction></div></AlertDialogContent></AlertDialog>
    </div>
  )
}
